...
const EntryList = ({ entries }: { entries: Entry[] }) => (
  <ul className="divide-y divide-border-secondary border-t border-border-secondary">
    {entries.map(entry => (
      <li key={entry.href}>
        <Link href={entry.href} className="group block py-6 transition-colors hover:bg-background-grain">
          {entry.meta && (
            <p className="text-xs font-medium uppercase tracking-[0.08em] text-text-secondary">{entry.meta}</p>
          )}
          ...
          <p className="mt-2 max-w-2xl text-[15px] leading-relaxed text-text-secondary">{entry.description}</p>
        </Link>
      </li>
    ))}
  </ul>
    ...
    <div className="bg-background-primary min-h-screen">
      <section className="pt-12 pb-14 md:pt-16 md:pb-20 bg-background-grain">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <h1 className="text-3xl md:text-5xl font-bold tracking-tight text-text-primary">Case studies</h1>
          <p className="mt-5 text-text-secondary text-lg md:text-xl leading-relaxed max-w-3xl">
            Longer write-ups of the work: what the problem was, what I decided, and what I would do differently.
          </p>
        </div>
      </section>
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-20 space-y-16 md:space-y-20">
        <section>
          <h2 className="mb-6 text-sm font-semibold uppercase tracking-[0.08em] text-text-secondary">Product work</h2>
        ...
        <section className="max-w-3xl">
          <h2 className="mb-4 text-sm font-semibold uppercase tracking-[0.08em] text-text-secondary">Also live</h2>
          <p className="text-[15px] leading-relaxed text-text-secondary">
            ...
            is a structured spec format for AI coding tools. .gist.design files give Claude, Cursor, and Copilot the
            design context they otherwise lack.
          </p>
        </section>

        <section>
          <Link href="/" className="text-sm font-medium text-text-secondary hover:text-accent transition-colors">
            Back to home
          </Link>
        </section>
      </main>
